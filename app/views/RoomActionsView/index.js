...
import { View, Text, Alert, Share, Switch } from "react-native";
...
class RoomActionsView extends React.Component {
  ...
  toggleEncrypted = async () => {
    ...
    try {
      ...
      try {
        ...
        const { result } = await RocketChat.saveRoomSettings(rid, {
          encrypted,
        });
  ...
  sendNotification = async (msg, onlyAudio) => {
    try {
      ...
      newMembers.map((member) => {
        ...
        this.getInfoOfUser(msg, member._id, onlyAudio);
      });
    } catch (e) {
      log(e);
    }
  ...
  getInfoOfUser = async (msg, IDUser, onlyAudio) => {
    try {
  ...
  sendPushNotificationWithCustomPayload = async (
    msg,
    devicetoken,
    os,
    onlyAudio
      ...
      return (
        <List.Section>
          ...
          <List.Item
            ...
            left={() => (
              <List.Icon name="ignore" color={themes[theme].dangerColor} />
            )}
            showActionIndicator
      ...
      return (
        <List.Section>
          ...
          <List.Item
            ...
            left={() => (
              <List.Icon name="logout" color={themes[theme].dangerColor} />
            )}
            showActionIndicator
            color={themes[theme].dangerColor}
          />
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withTheme(withDimensions(RoomActionsView)));
