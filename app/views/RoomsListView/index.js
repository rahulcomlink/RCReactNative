...
class RoomsListView extends React.Component {
  ...
  getSubscriptions = async () => {
    ...
    if (sortBy === "alphabetical") {
      defaultWhereClause.push(
        Q.experimentalSortBy(`${this.useRealName ? "fname" : "name"}`, Q.asc)
      );
    } else {
  ...
  toggleFav = async (rid, favorite) => {
    logEvent(
      favorite ? events.RL_UNFAVORITE_CHANNEL : events.RL_FAVORITE_CHANNEL
    );
    try {
      const db = database.active;
  ...
  renderSectionHeader = (header) => {
    ...
    return (
      ...
      >
        <Text style={[styles.groupTitle, { color: themes[theme].controlText }]}>
          {I18n.t(header)}
        </Text>
      </View>
    );
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withDimensions(withTheme(withSafeAreaInsets(RoomsListView))));
