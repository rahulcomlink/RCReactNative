...
class RoomView extends React.Component {
  ...
  constructor(props) {
    ...
    const roomUserId =
      props.route.params?.roomUserId ?? RocketChat.getUidDirectMessage(room);
    this.state = {
      joined: true,
      room,
      roomUpdate: {},
    ...
    try {
      ...
      if (this.tmid) {
        ...
        if (joined) {
          ...
          RocketChat.readMessages(room.rid, newLastOpen, true).catch((e) =>
            console.log(e)
          );
        }
      }
  ...
  updateUnreadCount = async () => {
    ...
    this.queryUnreads = observable.subscribe((data) => {
      ...
      const newUnreadsCount = data
        .filter((s) => s.unread > 0)
        .reduce((a, b) => a + (b.unread || 0), 0);
      if (unreadsCount !== newUnreadsCount) {
        this.setState({ unreadsCount: newUnreadsCount }, () =>
          this.setHeader()
        );
      }
    });
  ...
  handleRoomRemoved = ({ rid }) => {
    ...
    if (rid === this.rid) {
      ...
      showErrorAlert(
        I18n.t("You_were_removed_from_channel", {
          channel: RocketChat.getRoomTitle(room),
        }),
        I18n.t("Oops")
      );
    }
  ...
  getThreadMessages = () =>
    RocketChat.loadThreadMessages({ tmid: this.tmid, rid: this.rid });
  ...
  toggleFollowThread = async (isFollowingThread, tmid) => {
    try {
      await RocketChat.toggleFollowMessage(
        tmid ?? this.tmid,
        !isFollowingThread
      );
      EventEmitter.emit(LISTENER, {
        message: isFollowingThread
    ...
    if (!joined && !this.tmid) {
      return (
        ...
        >
          ...
          >
            <Text
              style={[styles.joinRoomText, { color: themes[theme].buttonText }]}
    ...
    return (
      ...
      >
        ...
        <List
          ...
          navigation={navigation}
          hideSystemMessages={
            Array.isArray(sysMes) ? sysMes : Hide_System_Messages
          }
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withDimensions(withTheme(withSafeAreaInsets(RoomView))));
